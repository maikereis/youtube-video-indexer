...
class RetryableOperation:
    ...
    def __init__(
        self,
        config: RetryConfig,
        non_retry_exceptions: Optional[List[Union[type, str]]] = None,
    ):
        self.config = config
        ...
        for attempt in range(1, self.config.max_attempts + 1):
            try:
                ...
                if self._is_non_retryable(e):
                    logger.warning(
                        f"{operation_name} skipped due to non-retryable error: {e}"
                    )
                    return None
                ...
                logger.warning(
                    f"{operation_name} failed on attempt {attempt}, retrying in {total_delay:.2f}s: {str(e)}"
                )
